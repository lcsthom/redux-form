...
function createReducer(structure) {
  ...
  var behaviors = (_behaviors = {}, _behaviors[_actionTypes.ARRAY_INSERT] = function (state, _ref2) {
    ...
    if (initial === undefined && payload === '' || payload === undefined) {
      result = deleteInWithCleanUp(result, "values." + field);
    } else if ((0, _isFunction["default"])(payload)) {
    ...
    var payload = _ref18.payload,
        ...
        updateDeepValues = _ref18$meta.updateDeepValues;
    ...
    if (keepDirty && registeredFields) {
      if (!deepEqual(newInitialValues, previousInitialValues)) {
        ...
        var overwritePristineValue = function overwritePristineValue(name) {
          ...
          var newInitialValue = getIn(newInitialValues, name); // If no new initial value
          ...
          if ((0, _isPlainObject["default"])(newInitialValue)) {
            Object.keys(newInitialValue).forEach(function (key) {
              overwritePristineValue(name + "." + key);
            });
          } else if (typeof (newInitialValue == null ? void 0 : newInitialValue.keySeq) === 'function') {
            // immutable-js case
            newInitialValue.keySeq().toArray().forEach(function (key) {
              overwritePristineValue(name + "." + key);
            });
          } else if (typeof (newInitialValue == null ? void 0 : newInitialValue.forEach) === 'function') {
            newInitialValue.forEach(function (item, index) {
          ...
          else if ((previousValue === previousInitialValue || !previousInitialValue) && previousValue !== newInitialValue) {
